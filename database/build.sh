...
docker build \
  -t ${CTIMS_DB_CONTAINER_IMAGE_LOCATION}:latest \
  -t ${CTIMS_DB_CONTAINER_IMAGE_LOCATION}:${GIT_REF} \
  -t ${CTIMS_DB_CONTAINER_IMAGE_LOCATION}:${COMMIT_ISH} \
  ...
  --label ca.uhn.techna.${PROJECT}.containerRole=database \
  $SELFDIR
