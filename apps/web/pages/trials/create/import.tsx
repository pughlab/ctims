...
const EditorImportTrialPage = () => {
  ...
  const trialId_1 = useSelector((state: RootState) => state.context.trialId);
  console.log("Saved value in Import", trialId_1);

  useEffect(() => {
    getCtmlSchemaOperation();
    const formData = sessionStorage.getItem('imported_ctml');
    console.log("Entered /trials/import")
    if (formData) {
...
console.log("trial.trial_id", trial.trial_id, trial.nct_id, editTrialObject);
