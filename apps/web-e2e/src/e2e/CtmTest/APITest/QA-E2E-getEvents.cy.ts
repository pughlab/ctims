import baseClass from "../../Base/baseClass.cy";
import dateClass from "../../Base/dateClass.cy"
describe("API Test",{ testIsolation: false }, () => {
  baseClass.beforeClass()
  let accesstoken= 'eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICI0VDVHc2dHLThzTl82NlNvX21kTnVwR2xQNHZ6QnVDcm9wMnJtb3Q2engwIn0..F2cfKN9BUPTU3dqec4CGERrXr84WKVmnZPPQimuH5F3ekEPB9_tsidFBB9_UpCP9vXM-X3plSDf5Rn1xN44ALyzal_hbdbZN20hSxPDshbvFCKt1xw7aLyRtPaSOf9d6VZ-cBVih2yCrv8s9yKBjBZ2WE8KUkqxpI7TpxPzQuRHKdsaZ_nz6e-mL4GQ15R3OTBQ5V46pxL5XKxBbuSwnZv9GndLUKc5jyfSxcpGt6RwCx-PW63fmI0ce0isNSjsNQp1EIXKOTKX-hGRJZ6VFym2jhvnUB9jPHDbVyCNk7pONNlGAu7W8MOyDLd1o2ILATYJuxhQQTlbNaN2px1g8OQ'

  it('should validate the API endpoint result', () => {
   cy.request({
     method: 'GET',
     url: 'https://ctims-api.qa02.technainstitute.net/events',
     headers: {
       'Authorization': 'Bearer' +accesstoken
     }
   }).then((res) => {
    /* const expectedTimestamp = new Date().toISOString();
     cy.log(expectedTimestamp)*/
     expect(res.status).to.eq(201)
    /* const resArray = res.body
     resArray.forEach((resBody) => {
       expect(resBody).to.have.property('type', 'LoginSuccessful');
       expect(resBody).to.have.property(dateClass.currentDate())
       if(resBody === dateClass.currentDate()) {
         cy.log(dateClass.currentDate())
         cy.log(JSON.stringify(resBody))
         expect(resBody).to.have.property('type', 'LoginSuccessful');
       }
     })*/
   })
  });
});
