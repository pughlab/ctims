...
import {NCT04293094_AMG650} from "./NCT04293094_AMG650";
...
describe('Validate as TrialGroupx Admin on "NCT04293094_AMG650" ', { testIsolation: false }, () => {
  baseClass.adminTrialGroupx()
  deleteDownloadsFolderBeforeAll()
  const ctmlTestData = NCT04293094_AMG650
  const ctmlJson = `./cypress/downloads/${jsonFile}`
  const ctmlYaml = `./cypress/downloads/${yamlFile}`

  it('should "Delete" the existing Ctml file "NCT04293094_AMG650" as Admin', () => {
    cy.deleteExistingTrial('NCT04293094_AMG650 TrialGroupx Admin role')
  })

 /* it('should enter values into the "Trial Editor Form" of "NCT04293094_AMG650" as Admin', () => {
    enterTrialEditorFormData(ctmlTestData,
      "NCT04293094_AMG650 TrialGroupx Admin role",
      "Srimathi",
      "Draft")
  })*/
  it('should enter values into the "Trial Editor Form" of "NCT04293094_AMG650" as Admin', () => {
  ...
  it.skip('should enter the Prior Treatment Requirement values of NCT04293094_AMG650 as Admin', () => {
  ...
  it('should enter the Age values of NCT04293094_AMG650 as Admin', () => {
  ...
  it('should enter the Drug List values of NCT04293094_AMG650 as Admin', () => {
  ...
  it('should enter the Management Group List values of NCT04293094_AMG650 as Admin', () => {
  ...
  it('should enter the Site List values of NCT04293094_AMG650 as Admin', () => {
...
//************ Arm 1  *****************
  it('should enter the values in "Treatment List and Matching criteria modal" for Arm 1 "NCT04293094_AMG650" as Admin' , () => {
    ...
    getMultipleArm().each(($input, index) => {
      ...
      if (index === 0) {
        ...
        let orConditions = ctmlTestData.treatment_list.step[0].arm[index].match[0].or[0]
        cy.enterSingleGenomicConditions(orConditions)

        cy.clickParentNode(0).click()
        cy.clickAnd()
        cy.clickParentNode(2).click()
        let andCondition = ctmlTestData.treatment_list.step[0].arm[index].match[0].or[1].and[0]
        cy.enterSingleClinicalCondition(andCondition)
        //cy.clickChildToggleArrowButton(0)
       //And
        cy.clickParentNode(0).click()
        cy.clickOr()
        cy.clickParentNode(4).click()
        let clinicCondition1 = ctmlTestData.treatment_list.step[0].arm[index].match[1].and[0].or[0]
        cy.enterSingleClinicalCondition(clinicCondition1)

        cy.clickParentNode(4).click()
        let clinicCondition2 = ctmlTestData.treatment_list.step[0].arm[index].match[1].and[0].or[1]
        cy.enterSingleClinicalCondition(clinicCondition2)

        cy.clickParentNode(4).click()
        let clinicCondition3 = ctmlTestData.treatment_list.step[0].arm[index].match[1].and[0].or[2]
        cy.enterSingleClinicalCondition(clinicCondition3)
  ...
  it('should enter the values in Arm 2', () => {
    ...
    getMultipleArm().each(($input, index) => {
      if (index === 1) {
        ...
        //!******** AND ********************
        cy.clickParentAnd()
        let clinicalCondition = ctmlTestData.treatment_list.step[0].arm[index].match[0]
