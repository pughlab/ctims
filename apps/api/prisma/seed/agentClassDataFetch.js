...
async function main(){
    return  new Promise((resolve, reject)=>{
        try{
            const result = [];
            fs.createReadStream("apps/api/prisma/metadata/nciagent/nciagentclass.csv")
                .pipe(csvParser())
                .on("data", (data) => {
                    result.push(data);
                })
            ...
            console.log("Unable to import agenmt class data from CSV into the 'agent class' table due to the following error: ",err)
            reject(err);
        }
    });
}
