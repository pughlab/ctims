...
describe('data-copy', () => {
  ...
  it('should exit if destination DB connection fails', async () => {
    mockTrialGroup.findMany.mockResolvedValueOnce([{ id: 1, name: 'Group A' }]);
    promptsMock.mockResolvedValueOnce({ selection: 1 });
    promptsMock.mockResolvedValueOnce({ databaseUrl: 'some-url', encryptionKey: 'some-key' });
  ...
  it('should successfully copy data to an empty destination DB', async () => {
    ...
    const sourceUsers = [{ id: 101, email: 'user1@example.com' }];
    const sourceSchemas = [{ id: 201, version: '1.0' }];
    const sourceTrials = [
      {
        id: 301,
        nct_id: 'NCT001',
        ...
        modifiedById: 101,
        userId: 101,
        ctml_schemas: [{ id: 201, version: '1.0' }],
        ctml_jsons: [{ id: 401, trialId: 301, versionId: 201, content: '{}' }],
      },
    ];

    // Mock prompts
    promptsMock.mockResolvedValueOnce({ selection: 1 });
    promptsMock.mockResolvedValueOnce({ databaseUrl: 'dest-url', encryptionKey: 'dest-key' });
